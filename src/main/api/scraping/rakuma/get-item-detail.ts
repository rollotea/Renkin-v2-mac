...
export async function getItemDetail(
  ...
  const name = await getName(page, filters.forbiddenString);
  const price = await getPrice(page);
  const seller = await getSeller(page, filters.seller);
  const description = await getDescription(page, filters.forbiddenString);
