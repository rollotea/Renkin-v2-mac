...
export async function getItemDetail(
  ...
  const name = await getName(page, presetFilters.forbiddenString);
  const price = await getPrice(page);
  const seller = await getSeller(page, presetFilters.seller);
  const description = await getDescription(page, presetFilters.forbiddenString);
