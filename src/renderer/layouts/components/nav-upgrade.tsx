...
export function NavUpgrade({ sx, ...other }: BoxProps) {
  ...
  const result: AuthContextValue = useAuthContext();
    ...
    >
      ...
      >
        <Box sx={{ position: "relative" }}>
          ...
          <Label
            color={
              (user?.plan === "Standard" && "info") ||
              (user?.plan === "Plus" && "warning") ||
              (user?.plan === "Unlimited" && "secondary") ||
              "info"
            }
            // color="success"
            variant="filled"
          ...
          ...theme.mixins.bgGradient({
            images: [
              `linear-gradient(135deg, ${varAlpha(
                ...
                0.92
              )}, ${varAlpha(theme.vars.palette.secondary.darkChannel, 0.92)})`,
