...
export function JobFiltersResult({ filters, totalResults, sx }: Props) {
  ...
  const handleRemoveEmploymentTypes = useCallback(
    (inputValue: string) => {
      const newValue = currentFilters.employmentTypes.filter(
  ...
  const handleRemoveRoles = useCallback(
    (inputValue: string) => {
      const newValue = currentFilters.roles.filter(
        (item) => item !== inputValue
      );
  ...
  const handleRemoveLocations = useCallback(
    (inputValue: string) => {
      const newValue = currentFilters.locations.filter(
        (item) => item !== inputValue
      );
  ...
  const handleRemoveBenefits = useCallback(
    (inputValue: string) => {
      const newValue = currentFilters.benefits.filter(
        (item) => item !== inputValue
      );
  ...
  return (
    ...
    >
      <FiltersBlock
        ...
        isShow={!!currentFilters.employmentTypes.length}
      >
      ...
      <FiltersBlock
        label="Experience:"
        isShow={currentFilters.experience !== "all"}
      >
        <Chip
          {...chipProps}
          label={currentFilters.experience}
      ...
      <FiltersBlock
        label="Locations:"
        isShow={!!currentFilters.locations.length}
      >
