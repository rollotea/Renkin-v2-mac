...
import { ScrapingTemplateList } from "../job-list";
...
export function JobListView() {
  ...
  const renderFilters = () => (
    ...
    >
      <JobSearch
        redirectPath={(id: string) => paths.dashboard.job.details(id)}
      />
      ...
      <Box sx={{ gap: 1, flexShrink: 0, display: "flex" }}>
        <JobFilters
          ...
          options={{
            ...
            employmentTypes: JOB_EMPLOYMENT_TYPE_OPTIONS.map(
              (option) => option.label
            ),
            experiences: [
              "all",
  ...
  return (
    <DashboardContent>
      ...
      <ScrapingTemplateList scrapingTemplates={dataFiltered} />
    </DashboardContent>
  );
}
