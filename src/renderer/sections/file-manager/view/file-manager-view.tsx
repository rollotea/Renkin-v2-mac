...
export function FileManagerView() {
  ...
  const handleDeleteItem = useCallback(
    async (id: string) => {
      ...
      try {
        await window.electron.ipcRenderer.deleteItem(id); // データベースから削除
        toast.success("Delete success!");

        await fetchData(setTableData); // 🔥 削除後に最新のデータを取得
  ...
  const handleDeleteItems = useCallback(async () => {
    ...
    // table.onUpdatePageDeleteRows(dataInPage.length, dataFiltered.length);
    try {
      await window.electron.ipcRenderer.deleteItems(table.selected); // データベースから削除
      toast.success("Delete success!");
  ...
  const handleEnqueueItem = useCallback(
    async (id: string) => {
      // const deleteRow = tableData.filter((row) => row.id !== id);
      ...
      try {
        ...
        table.onUpdatePageDeleteRows(dataInPage.length, dataFiltered.length);
        const file = await window.electron.ipcRenderer.fetchFileById(id);
        if (file) {
          dispatch(addQueueFiles(file));
          toast.success("Enque success!");
        }
      } catch (error) {
        console.error("Error enqueuing files:", error);
        toast.error("Error enqueuing files.");
      }
    },
    [dataInPage.length, table, tableData]
  );

  const handleEnqueueItems = useCallback(async () => {
    try {
      // await window.electron.ipcRenderer.deleteItems(table.selected); // データベースから削除
      // toast.success("Delete success!");
  ...
  const renderList = () =>
    displayMode === "list" ? (
      <FileManagerTable
        ...
        onEnqueueRow={handleEnqueueItem}
        notFound={notFound}
