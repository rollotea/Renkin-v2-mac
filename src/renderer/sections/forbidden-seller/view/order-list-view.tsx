...
import { fetchAllForbiddenSeller } from "../api/fetch-data";
...
const TABLE_HEAD: TableHeadCellProps[] = [
  ...
  { id: "name", label: "Forbidden Seller" },
  { id: "empty-1", label: "", width: 120, align: "center" },
...
export function ForbiddenSellerListView() {
  ...
  const [tableData, setTableData] = useState<ForbiddenSellerRow[]>([]);
  useEffect(() => {
    fetchAllForbiddenSeller(setTableData);
  }, [newFilesDialog]);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const promise = window.electron.ipcRenderer.deleteForbiddenSeller(id);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const promise = window.electron.ipcRenderer.deleteForbiddenSellers(
        table.selected
      );
      toast.promise(promise, {
        loading: "Loading...",
    ...
    <>
      <DashboardContent>
        ...
        <Card>
          ...
          <OrderTableToolbar
            fetchAllForbiddenString={fetchAllForbiddenSeller}
...
function applyFilter({
  ...
  if (name) {
    inputData = inputData.filter(({ seller }) =>
      [seller].some((field) =>
