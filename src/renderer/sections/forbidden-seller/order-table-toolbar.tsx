...
import {
  ForbiddenSeller,
  ForbiddenSellerRow,
} from "../../../main/types/setting/forbidden-seller";
...
export function OrderTableToolbar({
  ...
  const [forbiddenSeller, setforbiddenSeller] = useState<string>("");
  ...
  const handleAddNewForbiddenString = async () => {
    ...
    try {
      const value: ForbiddenSeller = { seller: forbiddenSeller };
      await window.electron.ipcRenderer.createForbiddenSeller(value);
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("UNIQUE constraint failed")) {
          toast.error(`This seller already exists [${forbiddenSeller}]`);
          return;
        } else {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <TextField
            fullWidth
            value={forbiddenSeller}
            onChange={(event) => {
              // console.log(event?.target.value);
              setforbiddenSeller(event?.target.value);
            }}
            placeholder="Forbidden Seller"
            slotProps={{
              input: {
                startAdornment: (
                  <InputAdornment position="start">
                    <Iconify
                      icon="fluent:person-prohibited-16-regular"
                      sx={{ color: "text.disabled" }}
                    />
                  </InputAdornment>
                ),
