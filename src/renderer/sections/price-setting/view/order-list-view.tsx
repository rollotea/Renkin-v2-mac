...
import { fetchAllPriceSetting } from "../api/fetch-data";
...
const TABLE_HEAD: TableHeadCellProps[] = [
  ...
  { id: "name", label: "Range", width: 130 },
  { id: "empty-1", label: "Value", width: 120 },
...
export function PriceSettingListView() {
  ...
  const [tableData, setTableData] = useState<PriceSettingRow[]>([]);
  useEffect(() => {
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const promise = window.electron.ipcRenderer.deletePriceSetting(id);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const promise = window.electron.ipcRenderer.deletePriceSettings(
        table.selected
      );
      toast.promise(promise, {
        loading: "Loading...",
...
function applyFilter({
  ...
  if (name) {
    inputData = inputData.filter(({ range, value }) =>
      [range.toString(), value.toString()].some((field) =>
