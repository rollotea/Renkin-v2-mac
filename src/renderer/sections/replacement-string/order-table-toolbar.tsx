...
} from "../../../main/types/setting/replacement-string";
...
export function OrderTableToolbar({
  ...
  const handleAddNewForbiddenString = async () => {
    ...
    try {
      const value: ReplacementString = {
        target: replacementTarget,
        value: replacementValue,
      };
      await window.electron.ipcRenderer.createReplacementString(value);
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("UNIQUE constraint failed")) {
          toast.error(
            `This target string already exists [${replacementTarget}]`
          );
          return;
        } else {
            ...
            slotProps={{
              input: {
                startAdornment: (
                  <InputAdornment position="start">
                    <Iconify
                      icon="fluent:navigation-location-target-20-regular"
                      sx={{ color: "text.disabled" }}
                    />
                  </InputAdornment>
                ),
